...
public class Games extends JavaPlugin {
    ...
    Configuration conf;
    private final GamesPlayerListener playerListener = new GamesPlayerListener(
            this);
    private final GamesBlockListener blockListener = new GamesBlockListener(
            this);
    ...
    public void onEnable() {
        ...
        // tmp.registerEvent(Event.Type.BLOCK_BREAK, blockListener,
        // Priority.Normal, this);
        // tmp.registerEvent(Event.Type.BLOCK_PLACE, blockListener,
        // Priority.Normal, this);
    ...
    public void reload() {
        ...
        for (World world : getServer().getWorlds()) {
            ...
            Map<String, ConfigurationNode> nodes = conf.getNodes(String.format(
                    "boards.%s", world.getName()));
            if (nodes != null) {
                for (String node : nodes.keySet()) {
                    String type = nodes.get(node).getString("type", "gol");
                    if (type.equalsIgnoreCase("gol")) {
                        tmp.add(new GOL(node, world, null, null, true, this,
                                conf));
                    } else if (type.equalsIgnoreCase("tetris")) {
                        tmp.add(new Tetris(node, world, null, null, true, this,
                                conf));
                    }
                }
            }
    ...
    public boolean onCommand(CommandSender sender, Command cmd,
        ...
        if (sender instanceof Player) {
            ...
            if ((permissions == null && !pl.isOp())
                    || (permissions != null && !permissions.has(pl,
                            "games.admin"))) {
            ...
            if (l >= 1) {
                if (args[0].equalsIgnoreCase("gol")) {
                ...
                } else if (args[0].equalsIgnoreCase("addboard")
                        || args[0].equalsIgnoreCase("ab")) {
                    if (checkNode(store, pl))
                        if (l == 3) {
                            if (findBoard(args[2]) == null) {
                                store.board = null;
                                if (args[1].equalsIgnoreCase("gol")) {
                                    store.board = new GOL(args[2],
                                            store.pos1.getWorld(), store.pos1,
                                            store.pos2, false, this, conf);
                                } else if (args[1].equalsIgnoreCase("tetris")) {
                                    store.board = new Tetris(args[2],
                                            store.pos1.getWorld(), store.pos1,
                                            store.pos2, false, this, conf);
                                }
                                if (store.board != null) {
                                    boards.get(store.pos1.getWorld()).add(
                                            store.board);
                                    pl.sendMessage(ChatColor.GREEN
                                            + "New board created and selected.");
                                    store.type = null;
                                } else {
                                    pl.sendMessage(ChatColor.RED
                                            + "Invalid board-type!");
                                ...
                                pl.sendMessage(ChatColor.RED
                                        + "Board with that name already exists!");
                            }
                        } else {
                            pl.sendMessage("Please provide board-type and area-name.");
                        }

                } else if (args[0].equalsIgnoreCase("deleteboard")
                        || args[0].equalsIgnoreCase("db")) {
                    if (l == 2) {
                        Board tmp = findBoard(args[1]);
                        if (tmp != null) {
                            boards.get(tmp.startBlock.getWorld()).remove(tmp);
                            conf.removeProperty(String.format("boards.%s.%s",
                                    tmp.startBlock.getWorld().getName(),
                                    tmp.name));
                            conf.save();
                ...
                } else if (args[0].equalsIgnoreCase("selectboard")
                ...
                } else if (args[0].equalsIgnoreCase("listboards")
                    ...
                    reload();
                        ...
                        || args[0].equalsIgnoreCase("bi")) {
                    if (checkBoard(store.board, pl)) {
                        store.board.info(pl);
                    }
                } else if (args[0].equalsIgnoreCase("savestate")
                        || args[0].equalsIgnoreCase("ss")) {
                } else if (args[0].equalsIgnoreCase("move")
                        || args[0].equalsIgnoreCase("mv")) {
                    if (checkBoard(store.board, pl)) {
                        if (checkNode(store, pl)) {
                    ...
                    if (!handleGOLParam((GOL) store.board, store.type, pl,
                            args, false, store)) {
                        pl.sendMessage("No such command.");
                        help = true;
                    }
                } else if (store.board instanceof Tetris) {
                    if (!handleTetrisParam((Tetris) store.board, pl, args,
                            false)) {
                        pl.sendMessage("No such command.");
                        help = true;
                    }
            ...
            if (help || golHelp || tetrisHelp) {
                if (l == 0 || (!golHelp && !tetrisHelp)) {
                    sender.sendMessage(ChatColor.YELLOW
                            + String.format(
                                    "%s by TZer0 (tzer0.jan@gmail.com), v%s",
                                    pdfFile.getName(), pdfFile.getVersion()));
                    sender.sendMessage(ChatColor.YELLOW
                            + "[] denotes arguments, {} means optional, () are aliases");
                    sender.sendMessage(ChatColor.YELLOW
                            + "red commands require a selected board");
                    sender.sendMessage(ChatColor.GREEN + "Commands:");
                    sender.sendMessage(ChatColor.GREEN
                            + "(n)ode {[1/2]} - shows or sets nodes");
                    sender.sendMessage(ChatColor.GREEN
                    ...
                    sender.sendMessage(ChatColor.RED
                            + "(b)oard(i)nfo - shows info about the board");
                    sender.sendMessage(ChatColor.RED
                            + "(m)o(v)e - moves board to current nodes");
                    sender.sendMessage(ChatColor.GREEN
                            + "(r)eload - reloads settings");
                    sender.sendMessage(ChatColor.GREEN
                            + "/g gol for game of life-specific commands");
                    sender.sendMessage(ChatColor.GREEN
                            + "/g tetris for tetris-specific commands");
                } else if (golHelp) {
                    sender.sendMessage(ChatColor.YELLOW
                            + "Game of life-specific (select board first) commands:");
                    sender.sendMessage(ChatColor.YELLOW
                            + "red commands require a selected type");
                    sender.sendMessage(ChatColor.GREEN
                    ...
                    sender.sendMessage(ChatColor.GREEN
                            + "(d)efault [materialID] - sets a default cell-type");
                    sender.sendMessage(ChatColor.RED
                            + "(m)od(t)ype [cmin/cmax/smin/smax/aw/dw] [v] - mods a type");
                    sender.sendMessage(ChatColor.RED
                            + "(w)in(p)os {(s)et} - shows/sets redstone-torch placement after win.");
                } else if (tetrisHelp) {
                    sender.sendMessage(ChatColor.YELLOW
                            + "Tetris-specific (select board first) commands:");
                    sender.sendMessage(ChatColor.GREEN
                            + "(m)odify(p)roperties [block(min)/block(max)/(dir)ection] [val]");
    ...
    public boolean handleTetrisParam(Tetris board, Player pl, String[] args,
            boolean sign) {
        int l = args.length;
        if (l >= 1) {
            if (args[0].equalsIgnoreCase("start")) {
                ...
                board.stopGame(pl);
                return true;
            } else if (args[0].equalsIgnoreCase("modifyproperties")
                    || args[0].equalsIgnoreCase("mp")) {
                if (l >= 2) {
                    if (l == 3) {
                        if (args[1].equalsIgnoreCase("blockmax")
                                || args[1].equalsIgnoreCase("max")) {
                            board.blockmax = Math.max(toInt(args[2]),
                                    board.blockmin);
                        } else if (args[1].equalsIgnoreCase("blockmin")
                                || args[1].equalsIgnoreCase("min")) {
                            board.blockmin = Math.min(toInt(args[2]),
                                    board.blockmax);
    ...
    public boolean handleGOLParam(GOL board, CellType type, Player pl,
            String[] args, boolean sign, PlayerStorage store) {
        int l = args.length;
        if (l >= 1) {
            if (args[0].equalsIgnoreCase("iterateboard")
                    || args[0].equalsIgnoreCase("ib")) {
                ...
                return true;
            } else if (args[0].equalsIgnoreCase("circuitboard")
                    || args[0].equalsIgnoreCase("cb")) {
                board.CircuitBoard(pl.getLocation().getBlock());
                return true;
            } else if (args[0].equalsIgnoreCase("loadstate")
                    || args[0].equalsIgnoreCase("ls")) {
                return true;
            } else if (args[0].equalsIgnoreCase("modtype")
                    || args[0].equalsIgnoreCase("mt")) {
                if (checkType(type, pl)) {
                    if (l >= 2) {
                        if (l >= 3) {
                            int val = toInt(args[2]);
                            if (args[1].equalsIgnoreCase("survmin")
                                    || args[1].equalsIgnoreCase("smin")) {
                                type.survMin = val;
                            } else if (args[1].equalsIgnoreCase("survmax")
                                    || args[1].equalsIgnoreCase("smax")) {
                                type.survMax = val;
                            } else if (args[1].equalsIgnoreCase("creamin")
                                    || args[1].equalsIgnoreCase("cmin")) {
                                type.creaMin = val;
                            } else if (args[1].equalsIgnoreCase("creamax")
                                    || args[1].equalsIgnoreCase("cmax")) {
                                type.creaMax = val;
                            } else if (args[1].equalsIgnoreCase("addweakness")
                                    || args[1].equalsIgnoreCase("aw")) {
                                type.addOther(val, pl);
                            } else if (args[1]
                                    .equalsIgnoreCase("deleteweakness")
                                    || args[1].equalsIgnoreCase("dw")) {
                        ...
                        pl.sendMessage(ChatColor.RED
                                + "Please provide field and value.");
                    }
                }
                return true;
                ...
                return true;
            } else if (args[0].equalsIgnoreCase("selecttype")
                ...
                return true;
            } else if (args[0].equalsIgnoreCase("deletetype")
                    || args[0].equalsIgnoreCase("dt")) {
                if (l == 2) {
                    if (store.type == ((GOL) store.board)
                            .findCell(toInt(args[1]))) {
                        store.type = null;
                    }
                ...
                return true;
            } else if (args[0].equalsIgnoreCase("winpos")
                    || args[0].equalsIgnoreCase("wp")) {
                if (l >= 2
                        && (args[1].equalsIgnoreCase("set") || args[1]
                                .equalsIgnoreCase("s"))) {
                    board.winpos = pl.getLocation().getBlock();
                    board.save();
                    pl.sendMessage(ChatColor.GREEN + "Set!");
                } else if (l >= 2
                        && (args[1].equalsIgnoreCase("remove") || args[1]
                                .equalsIgnoreCase("r"))) {
                    board.winpos = null;
                    board.save();
    ...
    public void showBoards(Player pl, int page) {
        ...
        pl.sendMessage(ChatColor.GREEN
                + String.format("Boards %d to %d", Math.min(page * 10, numbrd),
                        Math.min((page + 1) * 10, numbrd)));
    ...
    public boolean checkBoard(Board board, Player pl) {
        if (board == null) {
            pl.sendMessage(ChatColor.RED
                    + "No board selected, please select one.");
            return false;
        }
        return true;
    }
    ...
    public boolean checkType(CellType type, Player pl) {
        if (type == null) {
            pl.sendMessage(ChatColor.RED
                    + "No type selected, please select one.");
            return false;
        }
        return true;
    }
    ...
    public void posInfo(Location pos, String name, Player pl) {
        if (pos != null) {
            pl.sendMessage(ChatColor.YELLOW
                    + String.format("%s: (%d, %d, %d) in %s", name,
                            pos.getBlockX(), pos.getBlockY(), pos.getBlockZ(),
                            pos.getWorld().getName()));
        } else {
            pl.sendMessage(ChatColor.RED
                    + String.format("%s is not set.", name));
        }
    }
